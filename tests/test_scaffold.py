...
class ScaffoldStatusTests(unittest.TestCase):
    def test_project_status_reports_c6c(self) -> None:
        status = build_scaffold_status()

        self.assertEqual(status["project"], "Caption Compass")
        self.assertEqual(status["gate"], "C6C")
        self.assertEqual(status["status"], "vision-observation-ready")
        ...
        self.assertIn("structured frame observation contract", status["implemented"])
        self.assertIn("stub vision observation mode", status["implemented"])
        self.assertIn("Fireworks vision provider seam", status["implemented"])
    ...
    def test_future_gate_behavior_is_not_implemented(self) -> None:
        ...
        self.assertIn("scene-core fusion", future_behavior)
