# Coding Prompt: C5-T01 - Accuracy and Tone Evaluator
...
- Use the C5 issue taxonomy from the execution doc.
- Flag invented facts, missing facts, unsupported inference, tone bleed, and unclear jokes.
- Return actionable rewrite hints without performing repair.
- Work in stub/test mode without network.

## Concrete Artifact Requirement

Produce an evaluator artifact:

```text
docs/artifacts/c5-evaluation.sample.json
```
...
- scene_core_id
- threshold values
- per-tone scores for factual_accuracy, tone_match, and clarity
- pass/fail status
- issue codes
- repair_eligible flags
- rewrite hints where applicable
...
Fine-tuning
Leaderboard claims
Changing generated captions

## Expected Files Changed

```text
evaluator contract/service, prompt template, tests, README update, docs/artifacts/c5-evaluation.sample.json
```
...
3. Read `docs/execution/05-accuracy-tone-evaluator.execution.md`.
...
10. suggested conventional commit: `feat(evaluator): score caption accuracy and tone`
