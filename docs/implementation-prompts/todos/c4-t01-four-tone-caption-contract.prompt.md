# Coding Prompt: C4-T01 - Four Tone Caption Contract
...
Generate four captions from the same factual scene core with explicit tone separation.

## Required Behavior

- Generate exactly four caption outputs: formal, sarcastic, humorous-tech, humorous-non-tech.
- Keep the same factual core stable across all four tones.
- Use tone rubrics that are obvious to an LLM judge.
- Avoid obscure references, private jokes, hidden context, and unsupported intent/emotion claims.
- Include `scene_core_id` in the caption result.

## Concrete Artifact Requirement

Produce a four-tone caption artifact:

```text
docs/artifacts/c4-four-tone-captions.sample.json
```
...
- tone rubric labels or short tone evidence notes
- unsupported_inferences carried forward or excluded
...
```bash
python -m pytest -k 'caption or tone or generation'
```
...
```text
caption schema/service, prompt template, tests, README update, docs/artifacts/c4-four-tone-captions.sample.json
```
...
3. Read `docs/execution/04-four-tone-caption-generation.execution.md`.
...
10. suggested conventional commit: `feat(captions): generate four judged tones`
