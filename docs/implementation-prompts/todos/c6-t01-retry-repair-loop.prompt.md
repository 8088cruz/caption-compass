# Coding Prompt: C6-T01 - Retry Repair Loop
...
Add one bounded repair pass for eligible captions and record before/after score artifacts.

## Required Behavior

- Run at most one repair attempt per failed eligible caption.
- Use evaluator issue codes and rewrite hints as repair inputs.
- Preserve `scene_core_id` and the original factual scene core.
- Re-evaluate repaired captions and accept only when thresholds improve or pass.
- Hard-stop on non-repairable issues from the C6 execution doc.

## Concrete Artifact Requirement

Produce a repair trace artifact:

```text
docs/artifacts/c6-repair-trace.sample.json
```

It must include:

- max_repair_attempts set to 1
- repair thresholds
- repair_history
- before.text and before.scores
- after.text and after.scores
...
```bash
python -m pytest -k 'repair or retry or evaluator'
```
...
```text
repair service, schema updates, tests, README update, docs/artifacts/c6-repair-trace.sample.json
```
...
3. Read `docs/execution/06-retry-repair-loop.execution.md`.
...
10. suggested conventional commit: `feat(captions): add bounded repair loop`
