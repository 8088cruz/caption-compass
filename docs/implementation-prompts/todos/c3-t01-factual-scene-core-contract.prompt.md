...
Create the factual scene core JSON contract and generation boundary from timestamped evidence anchors.

## Required Behavior

- Convert timestamped evidence anchors into a stable factual scene core contract.
- Separate observed facts from unsupported inferences.
- Include uncertainty or unknown markers instead of guessing.
- Preserve `scene_core_id` for downstream captions and evaluation.
- Support deterministic stub/test mode without network.

## Concrete Artifact Requirement

Produce a scene core contract artifact:

```text
docs/artifacts/c3-scene-core.sample.json
```

It must include:

- scene_core_id
- evidence_anchor_ids or timestamps
- observed_entities
- visible_actions
- setting_or_context when supported
...
```bash
python -m pytest -k 'scene_core or factual or contract'
```
...
Evaluator scoring
Repair loop
Audio transcription unless already available

## Expected Files Changed

```text
scene core schema/service, prompt contract, tests, README update, docs/artifacts/c3-scene-core.sample.json
```
...
3. Read `docs/execution/03-factual-scene-core.execution.md`.
...
10. suggested conventional commit: `feat(scene-core): define factual scene core contract`
