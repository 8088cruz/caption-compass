...
Add deterministic video ingestion and timestamped frame sampling with explicit evidence anchors.

## Required Behavior

- Accept a local video path or uploaded video input through the current app boundary.
- Extract timestamped frame evidence anchors.
- Return a public-safe JSON summary of sampled frames.
- Handle invalid files, missing ffmpeg/OpenCV support, and unsupported formats clearly.
- Do not infer scene meaning in this gate; only collect evidence anchors.

## Concrete Artifact Requirement

Produce a timestamped evidence artifact:

```text
docs/artifacts/c2-frame-evidence.sample.json
```

It must include:

- source video identifier or sanitized filename
- duration when available
- sampled frame timestamps
- frame artifact paths or stable generated identifiers
- extraction status and warnings

Use a tiny synthetic fixture when no real sample video is safe to commit.
...
```bash
python -m pytest -k 'video or frame or ingestion'
```
...
```text
video/frame module, fixture or stub, tests, README update, docs/artifacts/c2-frame-evidence.sample.json
```
...
3. Read `docs/execution/02-video-frame-ingestion.execution.md`.
...
10. suggested conventional commit: `feat(video): add timestamped frame sampling`
